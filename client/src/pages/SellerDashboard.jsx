...
export default function SellerDashboard() {
  ...
  const fetchPreOrders = async () => {
    ...
    try {
      const res = await API.get(`/api/preorder/seller/${sellerId}`, { headers });
  ...
  const fetchMaterialOrders = async () => {
    ...
    try {
      const res = await API.get(`/api/material-orders/seller/${sellerId}`, { headers });
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      await Promise.all([fetchProducts(), fetchProfile(), fetchOrders(), fetchMaterialOrders()]);
    await Promise.all([fetchProducts(), fetchProfile(), fetchOrders(), fetchMaterialOrders(), fetchPreOrders()]);
      setLoading(false);
    })();
  ...
  const handleProductSubmit = async (e) => {
    ...
    try {
      ...
      if (editingProductId) {
        await API.put(`/api/products/${editingProductId}`, payload, { headers });
        alert("Product updated!");
        setEditingProductId(null);
      } else {
        await API.post("/api/products/add", payload, { headers });
        alert("Product added!");
      }
  ...
  const handleOrderDone = async (orderId, itemId) => {
    ...
    try {
      await API.put(`/api/orders/${orderId}/item/${itemId}/done`, {}, { headers });
      await fetchOrders();
  ...
  const handleProfileUpdate = async () => {
    try {
      await API.put("/api/users/profile", profileForm, { headers });
  ...
  const handlePasswordChange = async () => {
    try {
      await API.put("/api/users/change-password", passwordForm, { headers });
  ...
  return (
    <div style={{ padding: "20px" }}>
      ...
      <div style={{ marginBottom: "20px" }}>
        ...
        <button style={tabBtn(tab === "materialOrders")} onClick={() => setTab("materialOrders")}>
          Material Pre-Orders
        </button>
      ...
      {tab === "materialOrders" && (
        <div>
          <h2>Material Pre-Order Requests</h2>

          {materialOrders.length === 0 ? (
            <p>No material pre-orders yet.</p>
          ) : (
            materialOrders.map((order) => (
              ...
              >
                ...
                <p>
                  <b>Payment Option:</b> {order.paymentOption}
                </p>
                <p>
                  <b>Material Cost:</b> Rs {order.materialCost}
                </p>
                <p>
                  <b>Order Status:</b> {statusBadge(order.status)}
                </p>
                {order.supplierNotes && (
                  <p>
                    <b>Supplier Notes:</b> {order.supplierNotes}
                  </p>
                )}
                {order.deliveryInfo && (
                  <p>
                    <b>Delivery Info:</b> {order.deliveryInfo}
                  </p>
                )}
                <p>
                  <b>Payment Status:</b>{" "}
      ...
      {/* Profile */}
        ...
        <div style={{ marginTop: 30 }}>
          <h3 style={{ color: "#cc6600" }}>PreOrder Requests</h3>
          {preOrders.length === 0 ? (
                        ...
                        <>
                          ...
                          <button
                            onClick={async () => {
                              ...
                              try {
                                await API.delete(`/api/preorder/${order._id}`, {
              ...
              <div className="modal-box">
                ...
                <form
                  onSubmit={async (e) => {
                    e.preventDefault();
                    try {
                      await API.patch(`/api/preorder/${paymentOrder}/pay`, {}, {
                        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` }
                      });
